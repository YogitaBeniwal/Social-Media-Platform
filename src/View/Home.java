...
public class Home {
	...
	public Home(User user, Database database) {
		...
		sideBar.add(Box.createVerticalStrut(3));
		SideButton logout = new SideButton("Logout", "logout", user, database, frame);
		for (MouseListener ml : logout.getMouseListeners()) {
		    logout.removeMouseListener(ml);
		}
		logout.addMouseListener(new MouseListener() {
		    ...
		    @Override
		    public void mouseExited(MouseEvent e) {
		    	logout.setBackground(GUIConstants.white);
		    }
		   
		    @Override
		    public void mouseEntered(MouseEvent e) {
		    	logout.setBackground(GUIConstants.hover);
		    }
